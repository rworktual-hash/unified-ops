...
| GET | `/api/legacy-metrics/ai-insights/history` | Read-only 30m / 60m / 2h / today series from `ai_server_metrics` (fallback `ai_server_metrics_history`). Click a host card to isolate; otherwise fleet average. CPU, RAM, disk, load, GPU. |
