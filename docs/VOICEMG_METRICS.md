...
- `GET /api/legacy-metrics/voicemg/history?range=5m|today&group=ai_ccaas|ccaas|all` — live time-window series from `metrics_calls` + `metrics_system` (no copy into Unified Ops). UI: 5m polls every 5s, today every 30s.
