...
**Legacy portal metrics:** Read-only MySQL sync from **server-management** (`10.180.1.222` — SSH **4204**, MySQL usually **3306**) — see `docs/LEGACY_METRICS.md`, `scripts/discover-legacy-mysql-via-ssh.py`, `scripts/inspect-legacy-metrics-db.py`. Live extras (no new tabs): `GET /api/legacy-metrics/ai-insights/extras` and `GET /api/legacy-metrics/voicemg/extras`. VoiceMG 5m/today history: `GET /api/legacy-metrics/voicemg/history`.
