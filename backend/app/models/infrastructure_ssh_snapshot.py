...
class InfrastructureSshSnapshot(Base):
    __tablename__ = "infrastructure_ssh_snapshots"
    ...
    kong_active: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    docker_active: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    redis_role: Mapped[str | None] = mapped_column(String(32), nullable=True)
    redis_connected_clients: Mapped[int | None] = mapped_column(Integer, nullable=True)
    redis_used_memory_bytes: Mapped[int | None] = mapped_column(BigInteger, nullable=True)
